...
fl, fr = s.series_h("R", "RF1", "4.7M", 120, 84)
...
# Socket basing from the factory layout (6330×3228): the driver's +213 V plate
# lands on socket pin 1 and the cathodyne's +270 V plate on pin 6, so V3A is
# datasheet unit 2 (1/2/3) and V3B unit 1 (6/7/8).
...
s.gnd(142.24, 185.42)
# bias supply: HT tap -> 6.8k -> selenium -> -40V node, 56k bleeder.
# The factory drawing runs the 6.8k from the winding's bias tap INTO the
# rectifier's + end (crop triage 5f4-D1-schem.png, 6352x3744); until 2026-09-10 this row put the
# rectifier on the tap and the resistor after it.
# (row lifted 12 mm from y=172.72 so nothing reaches the A4 title-block corner)
s.glabel("HT_TAP", 150.1, 160.72, 180)
l, r = s.series_h("R", "RB1", "6.8k", 155.0, 160.72)
s.wire(150.1, 160.72, l, 160.72)
s.wire(r, 160.72, 165.92, 160.72)          # under C17's ground flag: a wire, not a body
s.sym("DIODE_SS", "D1", "SEL", 171.0, 160.72, lx=-2.0, ly=4.6, rot=180, label_rot=0)
...
# not drawn.
pt = s.pt("T1", "8087", 205, 176, tap=True)
