...
s.wire(116, YN + 17.62, nl, YN + 17.62)
...
for rref, rval, xr, nxt, cref, xlab in [("RD1", "1k 1W", 109, 124, "CB", "B+2"),
    ...
    s.wire(r, RAIL, nxt - (2 if rref == "RD3" else 0), RAIL)   # the chain ends on its last can
...
s.wire(200, YB, 206.19, YB)
l, r = s.series_h("R", "RBF", "100k 5%", 210, YB)
s.wire(r, YB, 218, YB)
s.sym("DIODE_SS", "DBIAS", "Si", 223.08, YB, lx=-2.0, ly=-5.4, rot=180, label_rot=0)
...
s.glabel("AC N", 200, 214, 180)
s.wire(200, 214, 240, 214)
s.glabel("MAINS N", 240, 214, 0)
s.junction(210, 214)
