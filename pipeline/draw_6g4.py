...
and notes.md. The A-FJ Super is the brown Pro's circuit (pipeline/draw_6g5.py)
part for part in the preamp, the harmonic vibrato and the phase inverter; it
differs in its GZ34 rectifier and its 10K feedback resistor, and this sheet is
drawn the same way. Channel 1 runs across the top and channel 2 below it, each
with two inputs, a first stage on V1, a Bass/Treble network and Volume, and a
second stage on V2. The two first stages share one cathode pair and the two
second stages another — the factory sheet letters them C and D; they are the
KI and KD labels here. Channel 1's output feeds the harmonic vibrato: a
low-pass and a high-pass branch into the two sections of V4, whose plates
recombine at VMIX, where channel 2 joins them. The oscillator V3A and its
phase splitter V3B run along the bottom left; the phase inverter V5, the 6L6GC
pair and the output transformer on the right; the power supply and its dropper
chain along the bottom right.
...
source in the DC model. Heaters, the GZ34's filament winding and the standby
switch are omitted here (see the board drawing).
...
Until 2026-09-10 this sheet drew one bottle per channel stage, a bias-vary
tremolo feeding the -55 V line, and no vibrato pair.
"""
...
s.note('KI and KD are the two shared cathode pairs the factory sheet letters C and D: both first stages (V1) sit on one 820 Ω / 25 µF, both second stages (V2) on another.')
...
s.note('Heaters, the GZ34 filament winding, the standby switch and the second of the two 20 µF reservoir cans are omitted here — see the sources list and the board drawing.')

# ============================ CHANNEL 1 (top) =========================
Y1 = 62
s.caption('Channel 1 — V1A, tone network and Volume, V2A (into the vibrato)', 12, 20, 1.6)
inputs(Y1, "CH1", "R2a", "R1a", "RG1I")
t1a = s.triode("V1A", "7025", VX, Y1, unit=1)
s.wire(GB, Y1, t1a["g"][0], Y1)
s.plate_load("RL1I", "100k", t1a["p"], "BA")
cathode_pair(VX, Y1, "RKI", "CKI", "KI")
wx, wy = tone_stack(Y1, "CC1T", "RS1T", "CT1", "VRT1", "VRB1", "CB1T", "RSL1", "VRV1", "CBR1")
...
inputs(Y2, "CH2", "R2b", "R1b", "RG2I")
t1b = s.triode("V1B", "7025", VX, Y2, unit=2)
s.wire(GB, Y2, t1b["g"][0], Y2)
s.plate_load("RL2I", "100k", t1b["p"], "BA")
cathode_label(VX, Y2, "KI")
wx, wy = tone_stack(Y2, "CC2T", "RS2T", "CT2", "VRT2", "VRB2", "CB2T", "RSL2", "VRV2", None)
t2b = s.triode("V2B", "7025", XB, Y2, lx=6.0, ly=-6.4, unit=1)
s.wire(wx, wy, wx, Y2)
s.wire(wx, Y2, t2b["g"][0], Y2)
s.plate_load("RL2D", "220k", t2b["p"], "BA")
cathode_label(XB, Y2, "KD")
tee2 = Y2 - 7.62 - 3.48
s.junction(XB, tee2)
...
s.wire(XJ, JY, tl, JY)
s.junction(XJ, JY)
s.sym("R", "RGPA", "1M", XJ, JY - 8, lx=-9.4)
s.wire(XJ, JY - 4.19, XJ, JY)
s.wire(XJ, JY - 11.81, XJ, YPH)
s.junction(XJ, YPH)
s.sym("R", "RGPB", "1M", XJ - 4, JY + 8, lx=-9.4)
...
s.wire(tl, JY, NFX, JY)
s.junction(NFX, JY)
l, r = s.series_h("R", "RNFB", "10k", NFX - 8, JY)
s.wire(r, JY, NFX, JY)
...
s.sym("R", "RPF", "1.5k", NFX, TAP + 7.31, lx=2.8, ly=0.0)
...
plates = {}
for gy, tee, vref, glref, scref in ((YPH, teeH, "V7", "RGL1", "RS1"),
                                    (YPB, teeC, "V8", "RGL2", "RS2")):
    ...
    s.gnd(XOUT, p["k"][1])
XT2 = 402
ot = s.ot_pp("T2", "45216", XT2, 98, lx=-6.35, ly=-14.5)
...
s.glabel("BP1", XT2 - 17, ot["ct"][1], 180)
s.wire(*ot["sec_h"], XT2 + 13, ot["sec_h"][1])
s.glabel("SPKR", XT2 + 13, ot["sec_h"][1], 0)
s.wire(*ot["sec_c"], XT2 + 13, ot["sec_c"][1])
s.glabel("GND", XT2 + 13, ot["sec_c"][1], 0)
s.text("Two 10-inch speakers", XT2 + 6, 90, 1.1)

# ============================ POWER SUPPLY ==============================
s.caption("Power supply — GZ34 full-wave, CH-125C1A choke, and the dropper chain to every rail", 196, 186, 1.4)
BY = 213
# A-FJ: each HT end of T1 (8087) runs to one GZ34 plate; the rectifier's
# cathode (pin 8) is the reservoir; the HT centre tap is grounded.
s.glabel("HT_A", 222, BY - 22, 90)
s.wire(222, BY - 22, 222, BY - 19.5)
s.diode_tube("V9A", "GZ34", 222, BY - 11.88, lx=-11.8)
s.glabel("HT_B", 234, BY - 22, 90)
s.wire(234, BY - 22, 234, BY - 19.5)
s.diode_tube("V9B", "GZ34", 234, BY - 11.88, lx=6.2)
s.wire(222, BY - 4.26, 222, BY)
s.wire(234, BY - 4.26, 234, BY)
s.wire(222, BY, 244, BY)
s.junction(234, BY)
s.junction(244, BY)
s.sym("C", "C10", "20u 600V", 244, BY + 3.81, lx=2.6, ly=1.6)
s.gnd(244, BY + 7.62)
s.wire(244, BY, 244, BY - 3.5)
s.glabel("B_RES", 244, BY - 3.5, 90)
s.wire(244, BY, 248.38, BY)
s.sym("CHOKE", "CH1", "125C1A", 256, BY)
...
# power transformer: primary to the mains labels, HT to the GZ34, bias tap
PTX, PTY = 352, 222
pt = s.pt("T1", "8087", PTX, PTY, lx=-6.35, ly=-11.9, tap=True)
s.wire(pt["tap"][0], pt["tap"][1], pt["tap"][0] + 2, pt["tap"][1])
s.glabel("HT_TAP", pt["tap"][0] + 2, pt["tap"][1], 0)
s.wire(*pt["pri1"], PTX - 14, PTY - 5.08)
s.glabel("MAINS", PTX - 14, PTY - 5.08, 180)
s.wire(*pt["pri2"], PTX - 14, PTY + 5.08)
s.glabel("MAINS N", PTX - 14, PTY + 5.08, 180)
...
# bias supply, read off A-FJ: T1's bias tap -> rectifier (+ end toward the
...
YBI = 258
s.caption("Bias supply — T1's bias tap, a rectifier, 8 µF, 10k, then 8 µF with a 56k bleeder: -55 V", 196, 248, 1.3)
